...
class Atom:
    def __init__(self, segid='X', resid='N/A', resn='N/A', atomname='C', atomtype='N/A',
                 coordinate=np.array([0., 0., 0.]), mass=12.0, charge=0.0, hbond="none", hydro="", index=0):
        self.__segid = segid
        self.__resid = resid
        self.__resn = resn
    ...
    def __repr__(self):
        return "<Atom> segid " + str(self.__segid) + ", resid " + str(self.__resid) + ", resn " + str(self.__resn) + \
               ", index " + str(self.__index) + ", name " + self.__name + ", type " + self.__type + \
               ", coordinate " + \
    ...
    for segment in cave.segments:
        ...
        for i, res in enumerate(seg.residues):
            ...
            for atom in atoms:
                ...
                atomindex = atom.id # Note: atomindex = atom.index will cause confusion
                atomname = atom.name
                ...
                a = Atom(segid=segid, resid=resi, resn=resn, atomname=atomname, atomtype=atomtype, coordinate=coor,
                         mass=mass, charge=charge, hbond=hbond, hydro=hydro, index=atomindex)
                info.append(a)
